...
def criar_support_point():
    ...
    for _ in range(num_trabalhadores):
        ...
        verify_user = verify_users(cpf_trab)
        
        if(verify_user == None):
            print("Usuário não cadastrado. Cadastre-o primeiro.")
        else:
            trabalhador = {
                "nome": verify_user.get("nome", "Desconhecido"),
                "função": verify_user.get("função", "Desconhecida")
            }
            trabalhadores[cpf_trab] = trabalhador


    num_animais = int(input("Quantos animais deseja adicionar? "))
    for id_animal in range(1, num_animais + 1):
        ...
        animais[str(id_animal)] = animal  # adiciona corretamente
...
def ler_support_points():
    ...
    for id, sp in encontrados.items():
        ...
        print("Trabalhadores:")
    if sp['trabalhadores']:
        for cpf, dados in sp['trabalhadores'].items():
            print(f"CPF: {cpf}")
    ...
    print("Animais:")
    if sp['animais']:
        for id_animal, dados_animal in sp['animais'].items():
            print(f"\nAnimal {id_animal}:")
            for chave, valor in dados_animal.items():
                print(f"{chave.capitalize()}: {valor}")
    else:
